...
class NormalAssertion extends BaseAssertion {
    ...
    /*
    __nCheck(bool, isFalseSuccess) {
        if (!this.__isBool(bool)) { this._consoleFail(this._M.msg.normal.exception.fnReturn, this._caller); return false; }
        if (isFalseSuccess ? bool : !bool) { this._consoleFail(this.__failMsg(isFalseSuccess), this._caller); return false; }
        return true
    }
//    if (this.__nCheck(bool, isFalseSuccess)) {this._count.success++} else {this._count.fail++}
    __nCheck(bool, isFalseSuccess) {
