...
//            console.log(this.stack)
            ...
            //console.error(msg + '\n', this.__delStacks(this.stack).join('\n'))
    ...
    _consoleError(msg, err, caller) {
        if (Error.captureStackTrace) {
            ...
            const init = []
            //const errsNl = [msg, ...errs].reduce((a,v)=>{a.push(v.toString());a.push('\n');return a;},init).filter(v=>-1===v.indexOf('assertion.js:'))
            //const errsNl = [msg, ...errs].reduce((a,v)=>{a.push(v.toString());a.push('\n');return a;},init).filter(v=>{console.log(v);return -1===v.indexOf('assertion.js:')})
...
//            console.log(stacks.join('\n'))
            console.error(msg, '\n', stacks.join('\n'))
            //console.error([msg, ...stacks].join('\n'))
//            console.log([msg, ...errs].reduce((a,v)=>{a.push(v.toString());a.push('\n');return a;},init))
//            console.error(...this.__delStacks([msg, ...errs].reduce((a,v)=>{a.push(v.toString());a.push('\n');return a;},init)))
        } else { console.error(msg) }
    }
    ...
    //__delStacks(stacks) { return stacks.filter(line=>-1===line.indexOf('assertion.js:')) }
    __delStacks(stacks) {
...
//        if (this.__isStr(stacks)) { stacks.split('\n') }
//        return stacks.filter(line=>-1===line.indexOf('assertion.js:')) }
}
