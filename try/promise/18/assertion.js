...
class Message {
    ...
    //get stt() { return this._status[(this._message.hasOwnProperty(this._lang) ? this._lang : 'ja')] }
    ...
    get status() { return {
        pending:   {color:{f:'#666666',b:'#CCCCCC'}, ...this.stt.pending},
        exception: {color:{f:'#0000AA',b:'#99CCFF'}, ...this.stt.exception},
        fail:      {color:{f:'#AA0000',b:'#FFBEDA'}, ...this.stt.fail},
        success:   {color:{f:'#008800',b:'#AEFFBD'}, ...this.stt.success},
    }}
}
...
//            console.log(`%c${MSG}`, `background-color:#99CCFF;color:#0000AA;`)
        ...
        /*
        if (undefined===err || this.__isFn(err)) {
            
        }

        }
        //const MSG= msg + '\n' + (err ? this._getErrorStacks(err) : this._captureStacks(this._console)).join('\n')
        const MSG= msg + '\n' + (err ? this._getErrorStacks(err) : this._captureStacks(this._console)).join('\n')
        if (['exception','fail'].some(s=>s===status)) {
//            console.log(`%c${MSG}`, `background-color:#99CCFF;color:#0000AA;`)
            console.log(`%c${MSG}`, `background-color:${this._M.stt[status].b};color:${this._M.stt[status].f};`)
        } else {
            console.log(MSG)
        }
        */
...
//            if (eMsg) {this._console('fail', eMsg, this._caller)}
            this.__nCheck(bool)
        } catch(err) {
    ...
    _nB(fn) {
        ...
        if (this._isFalseSuccess ? fn : !fn) {
            this._count.fail++
            //this._consoleFail(this.__failMsg(), this._caller)
...
//        console.log(bool, this._isFalseSuccess)
        ...
        if (!this.__isBool(bool)) {
            this._count.exception++
            //return this._consoleFail(this._M.msg.normal.exception.fnReturn, this._caller)
            return this._console('fail', this._M.msg.normal.exception.fnReturn, this._caller)
        }
        // テスト失敗：Eであるべき所がAです。
        else if (this._isFalseSuccess ? bool : !bool) {
            this._count.fail++
            //return this._consoleFail(this.__failMsg(), this._caller)
    ...
    */
    ...
    //__exceptMsg(err) { this._count.exception++; this._consoleException(this._M.msg.normal.exception.throw(this._M.cmn.return[this._isFalseSuccess ? 'false' : 'true']), err, this._caller) } // テスト例外。Bであるべき所で例外発生しました。
    __exceptMsg(err) { // テスト例外。Bであるべき所で例外発生しました。
        this._count.exception++
        //this._consoleException(this._M.msg.normal.exception.throw(this._M.cmn.return[this._isFalseSuccess ? 'false' : 'true']), err, this._caller)
    ...
    _eFn(type, msg, fn) {
        try {
            ...
            if (0===eMsgs.length) { this._count.success++; return true; }
                ...
                this._count.fail++;
                //this._consoleException(`${this._M.msg.exception.runtime.summary(eMsgs.length)}`+eMsgs.join('\n'), err, this.e)
...
class Assertion {
    ...
    fin(onFinishedAsync, onFinishedSync) {
        ...
        if (0 < this.count.pending) { // 非同期テスト実行＆完了後実行
            Promise.allSettled([
                ...
                for (let res of results) { // res.[status,value,reason]
                    ...
                    if ('rejected'===res.status) {
                        ...
                        //this._t._consoleException(this._M.msg.fin.exception.one, res.reason, this.fin)
            ...
            //.catch(err=>this._t._consoleException(this._M.msg.fin.exception.all, err, this.fin))
