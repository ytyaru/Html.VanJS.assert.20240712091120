...
class Assertion {
    constructor(lang) {
        ...
        console.log(this._f, this._f.assert)
    ...
    get count() { return this._count }
    test(options) {// {labe:'', setup:()=>, tearDown:(t)=>, method:'', assert:Assertion.t/f/e, inouts:[[[arg,...], (res)=>assertに渡す値], ...]}
        const OP = {
            setup:(args)=>{throw new TestError('setup未実装。テスト対象クラスのインスタンスを返す関数を実装してください。')},
            tearDown:()=>{},
            ...options}
        if (!OP.hasOwnProperty('method')) { throw new TestError(`テスト対象メソッド名を文字列で入力してください。`) }
        for (let io of options.inouts) {
            const target = options.setup()
            //if (!target.hasOwnProperty(OP.method)) {
            if (!(OP.method in target)) {
                console.error(target)
                throw new TestError(`テスト対象メソッド名がテスト対象インスタンス内に存在しません。:${target.constructor.name}.${OP.method}`);  }
            const [args, test] = io
            if (OP.hasOwnProperty('assert')) { // options.assert がある場合
                ()=>target[OP.method](...args)
                const testRes = test(target[OP.method](...args))
                console.log(testRes, OP.assert, OP)
                if (Array.isArray(testRes)) { OP.assert(...testRes) }
                else { OP.assert(testRes) }
            } else {
                test(target[OP.method](...args)) // options.assert がない場合（テストコード内でt/f/eを実行する）
            }
            options.tearDown(target)
        }

        /*
        for (let io of options.inouts) {
            const target = options.setup()
            const M = OP.hasOwnProperty('method') ? OP.method : target.constructor
            const [args, test] = io
            if (options.hasOwnProperty('assert')) {
                options.assert(test(M(...args))) // options.assert がある場合
                //options.assert(test(target[options.method](...args))) // options.assert がある場合
            } else {
                test(M(...args)) // options.assert がない場合（テストコード内でt/f/eを実行する）
                //test(target[options.method](...args)) // options.assert がない場合（テストコード内でt/f/eを実行する）
            }
            options.tearDown(target)
        }
        */
    }
    testT(options) { return this._testN(false, options) }
    testF(options) { return this._testN(true, options) }
    _testN(isFalseSuccess, options) {
        const OP = {
            setup:(args)=>{throw new TestError('setup未実装。テスト対象クラスのインスタンスを返す関数を実装してください。')},
            tearDown:()=>{},
            ...options}
...
//                console.error(target)
                throw new TestError(`テスト対象メソッド名がテスト対象インスタンス内に存在しません。:${target.constructor.name}.${OP.method}`);  }
            const [args, test] = io
            const tf = isFalseSuccess ? this._f : this._t
            const fn = ('AsyncFunction'===(target[OP.method]).constructor.name)
                ? async()=>await test(target[OP.method](...args))
                : ()=>test(target[OP.method](...args))
            tf.assert(fn)
///            tf.assert(test(target[OP.method](...args)))
//            console.log((target[OP.method]).constructor.name)
//            const fn = ('function'!==typeof test) ? ()=>test(target[OP.method](...args)) : test
//            tf.assert(fn(target[OP.method](...args)))
            //if ('function'!==test) { test = ()=>test(target[OP.method](...args))}
            //tf.assert(()=>test(target[OP.method](...args)))
            //tf.assert(test(target[OP.method](...args)))
                /*
            try {
                if (OP.hasOwnProperty('assert')) { // options.assert がある場合
                    //const testRes = test(target[OP.method](...args))
                    const assert = isFalseSuccess ? this.f : this.t
                    assert(()=>test(target[OP.method](...args)))
    ...
    testE(options) {
        ...
        for (let io of options.inouts) {
            ...
            if (!(OP.method in target)) {
                console.error(target)
                throw new TestError(`テスト対象メソッド名がテスト対象インスタンス内に存在しません。:${target.constructor.name}.${OP.method}`);  }
            const [args, eType, eMsg] = io
            if (OP.hasOwnProperty('assert')) { // options.assert がある場合
                this.e(eType, eMsg, ()=>target[OP.method](...args))
//                const testRes = test(target[OP.method](...args))
//                console.log(testRes, OP.assert, OP)
//                if (Array.isArray(testRes)) { OP.assert(...testRes) }
//                else { OP.assert(testRes) }
