...
class BlackBox {
    constructor(assertion) {
        this._assertion = assertion ?? new Assertion()
    }
    get assertion() { return this._assertion }
    get defaultOptions() { return {class:{name:'',args:[]},method:'',assert:'t',inouts:[[], (r)=>r===true], tearDown:(t)=>{}} }
    test(options) {
        ...
        if (isAsync) {
//            throw new Error('未実装')
            this._testAsync(options, target, mthNm)
        } else { this._testSync(OP, target, mthNm) }
        OP.tearDown(target)
    }
    fin() { this._assertion.fin() }
    _getInstance(options) {
        const C = options.class
        if ('function'===typeof C) { return new C() }
        else if ('string'===typeof C || C instanceof String) { return new Reflect.construct(window[C], []) }
        const [N,A] = [C.name, this._getArgs(options)]
        console.log(N,A)
        if ('function'===typeof N) { return new N(...A) }
        else if ('string'===typeof N || N instanceof String) { return new Reflect.construct(window[N], A) }
        else { throw new TestError(`options.class.nameはクラス名またはコンストラクタを指定してください。`) }
    }
    _getArgs(options) { const A = options.class.args; return ('function'===typeof A) ? A() : A; }
    ...
    _testSync(options, target, mthNm) {
        for (let io of options.inouts) {
            ...
            this._assertion[options.assert](...this._testSyncAssArgs(options, target, mthNm, args, expected))
        }
    }
    ...
    /*
    _testSync(options, target, mthNm) { this[`_test${'e'===options.assert ? 'E' : 'N'}Sync`](options, target, mthNm) }
    _testNSync(options, target, mthNm) {
        for (let io of options.inouts) {
            const [args, expected] = io
            this._assertion[options.assert](()=>expected(target[mthNm](...args)))
        }
    }
    _testESync(options, target, mthNm) {
        for (let io of options.inouts) {
            const [args, expected] = io
            this._assertion[options.assert](...expected, ()=>target[mthNm](...args))
        }
    ...
    */
    _testAsync(options, target, mthNm) {
        for (let io of options.inouts) {
            const [args, expected] = io
            this._assertion[options.assert](
                ...('e'===options.assert
    ...
    //_testAsyncAssArgs(options, target, mthNm, args, expected) { return 'e'===options.assert ? [...expected, async()=>await target[mthNm](...args)] : [async()=>expected(await target[mthNm](...args))] }
//    _testAsyncAssArgs(options, target, mthNm, args, expected) {
    ...
    _testAsync(options, target, mthNm) {
        ...
        for (let io of options.inouts) {
            ...
            if ('e'===options.assert) {
                promises.push(new Promise((resolve,reject)=>{
                    target[mthNm](...args)
                    //.then(r=>resolve(expected(r)))
                    .then(r=>{
                            this._assertion[options.assert](...expected, async()=>R)
                        const R = expected(r)
                        if ('e'===options.assert) { this._assertion[options.assert](...R) }
                        else { this._assertion[options.assert](R) }
                        resolve(R)
                    })
                    .catch(e=>reject(`非同期テストで例外発生しました。: ${target.constructor.name}.${mthNm}(${...args})`))
                }))
            } else {
            ...
            promises.push(new Promise((resolve,reject)=>{
                ...
                .then(r=>{
                    if ('e'===options.assert) {
                        this._assertion[options.assert](...expected, ()=>R)
                    } else {
//                        const R = expected(r)
//                        this._assertion[options.assert](R)
                        this._assertion[options.assert](()=>expected(r))
                    }
                    const R = expected(r)
                    if ('e'===options.assert) { this._assertion[options.assert](...R) }
