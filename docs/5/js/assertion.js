...
class BlackBox {
    ...
    test(options) {
        ...
        if (isAsync) { this._testAsync(options, target, mthNm) }
    ...
    _getInstance(options) {
        ...
        if ('function'===typeof C) { return new C() }
            ...
            if (0===C.length) { throw new TestError(`options.class.nameはクラス名またはコンストラクタを指定してください。`) }
                ...
                if ('function'===typeof C[0]) { return new C[0](...C.splice(1)) }
                else if ('string'===typeof C || C instanceof String) { return new Reflect.construct(window[C[0]], C.splice(1)) }
            }
        }
        else {
    ...
    _getInstance(options) {
        const C = options.class
        const [N,A] = [C.name, this._getArgs(options)]
        //const ins = this._getInsFnStr(C, args)
        const ins = this._getInsFnStr(C, A)
        //if (let ins = this._getInsFnStr(C, args)) { return ins }
        if (ins) { return ins }
        else if (Array.isArray(C)) {
            if (0===C.length) { throw new TestError(`options.class.nameはクラス名またはコンストラクタを指定してください。`) }
    ...
    _getInsFnStr(C, args) {
        if ('function'===typeof C) { return new C(...args) }
        else if ('string'===typeof C || C instanceof String) { return new Reflect.construct(window[C], args ?? []) }
        return false
    }
    //_getArgs(options) { const A = options.class.args; return ('function'===typeof A) ? A() : A; }
    _getArgs(C) {
        if ('function'===typeof C) { return [] }
        else if ('string'===typeof C || C instanceof String) { return [] }
        else if (Array.isArray(C)) {
            if (0 === C.length) { throw new TestError(`options.class.argsはクラス名またはコンストラクタを指定してください。`) }
            else if (1 === C.length) { return [] }
            //else { return C[1] } // 配列なら可変長引数。関数なら実行結果
            //else { return C.splice(1) }
            //else { return ('function'===typeof C[1]) ? C[1]() : C[1] } // 配列なら可変長引数。関数なら実行結果
            else {
            ...
            //const A = options.class.args
            const A = C.args
            if (!A) { return [] }
            if ('function'!==typeof A && !Array.isArray(A)) { throw new TestError(`options.class.argsはコンストラクタ引数を示す配列かそれを返す関数にしてください。`) }
...
//        return ('function'===typeof A) ? A() : A
    ...
    _validMethod(options, ins) {
        console.log(ins, options.method)
